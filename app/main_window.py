...
from PyQt5.QtWidgets import (QMainWindow, QWidget, QVBoxLayout, QHBoxLayout, 
                             ...
                             QProgressBar, QComboBox, QPushButton, QDialog)
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        # Show a loading message while backend initializes
        ...
        # Start background initialization shortly after the event loop starts
    ...
    def on_initialization_complete(self, config, templates, session):
        ...
        # Build the UI now that we have the core components
        self.create_actions()
        self.create_menus()
        self.create_toolbar()
    ...
    def setup_threads_post_init(self):
        """
        Creates and configures the persistent background threads for OCR and file importing.
        """
        ...
        # --- File Importer Thread (Corrected Setup) ---
        ...
        # When the worker signals it's finished, the thread can quit
        self.file_importer.import_finished.connect(self.importer_thread.quit)

    def start_file_import(self, files_to_import: list):
        """
        ...
        self.status_label.setText(f"Importing {len(files_to_import)} files...")
        # Set the list of files for the existing worker to process
        self.file_importer.set_files_to_import(files_to_import)
        
        # Start the existing, persistent thread
        self.importer_thread.start()

    def on_import_finished(self):
        """Slot to handle GUI updates after file import is complete."""
        self.status_label.setText("Import complete.")
        # Select the first file in the list if it's not empty
        if self.file_list.count() > 0:
            self.file_list.setCurrentRow(0)

    def create_main_layout(self):
        # This method's content remains the same as in your original file.
        ...
        self.file_list.files_dropped.connect(self.start_file_import)
    ...
    def on_file_selected(self, current_item, _=None):
        if current_item:
            file_path = current_item.data(Qt.UserRole)
            self.image_view.load_page(file_path)
            self.status_label.setText(f"Viewing: {os.path.basename(file_path)}")

    # --- Other methods (create_actions, menus, toolbar, etc.) ---
    # The content of these methods can remain largely the same as in your file.
    ...
    def create_menus(self):
        ...
        edit_menu.addAction(self.tesseract_prefs_action)
        view_menu = menubar.addMenu("&View")
        view_menu.addAction(self.zoom_fit_action)
        help_menu = menubar.addMenu("&Help")
        help_menu.addAction(self.check_deps_action)
    ...
    def create_toolbar(self):
        ...
        toolbar.addAction(self.process_files_action)
        toolbar.addAction(self.stop_process_action)
        toolbar.addSeparator()
        toolbar.addWidget(QLabel("OCR Engine:"))
        self.ocr_engine_combo = QComboBox()
        self.ocr_engine_combo.addItems(["windows", "tesseract", "pdf_native", "none"])
    ...
    def start_ocr_processing(self):
        ...
        self.stop_process_action.setEnabled(True)
        self.ocr_processor.files_to_process = self.file_list.get_all_paths()
        self.ocr_processor.templates_to_use = self.templates.get_templates()
        self.ocr_processor.ocr_engine = self.ocr_engine_combo.currentText()
    ...
    def on_ocr_finished(self):
        ...
        self.progress_bar.setVisible(False)
        self.process_files_action.setEnabled(True)
        self.stop_process_action.setEnabled(False)
    ...
    def create_new_template(self):
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            if self.templates.get_template_by_name(new_data['name']): 
                return QMessageBox.warning(self, "Duplicate Name", "A template with this name already exists.")
            self.templates.add_template(new_data)
            self.load_initial_templates()

    def edit_selected_template(self):
        selected_item = self.template_list_widget.currentItem()
        if not selected_item: 
            return QMessageBox.warning(self, "No Selection", "Please select a template to edit.")
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            self.templates.delete_template(selected_item.text())
            self.templates.add_template(updated_data) 
            self.load_initial_templates()
    
    def delete_selected_template(self):
        selected_item = self.template_list_widget.currentItem()
        if not selected_item: 
            return QMessageBox.warning(self, "No Selection", "Please select a template to delete.")
    ...
    def open_tesseract_preferences(self):
        ...
        if dialog.exec_() == QDialog.Accepted:
            psm, oem = dialog.get_selected_options()
            self.config.set('tesseract_psm', psm)
            self.config.set('tesseract_oem', oem)
            self.config.save_config()
            QMessageBox.information(self, "Saved", "Tesseract preferences have been saved.")
    ...
    def manual_dependency_check(self):
        ...
        if not messages: 
            QMessageBox.information(self, "Dependencies Check", "All dependencies appear to be installed correctly.")
        else: 
            QMessageBox.warning(self, "Dependencies Check Results", "<br><br>".join(messages))

    def closeEvent(self, event):
        # Cleanly shut down all threads
